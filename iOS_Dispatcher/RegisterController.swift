...
class RegisterController: UIViewController {
    ...
    func handleRegister() {
        ...
        let values = ["email": userEmail, "phoneNumber": userPhone, "name": userName, "password": password]
        Auth.auth().createUser(withEmail: userEmail, password: password) { (user, error) in
            ...
            let databaseRef = Database.database().reference().child("CER_dispatchers").child(uid)
